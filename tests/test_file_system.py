...
class TestFileSystem(TestCase):
    ...
    @patch('app.entities.FileManagement._get_root')
    def test_get_file_directory_when_directory_does_not_exist(self, mock_get_root) -> None:
        mock_get_root.return_value = '/tmp'
        with patch.object(Path, 'exists') as mock_exists:
    ...
    def test_generate_file(self, mock_file_directory) -> None:
        with patch.object(Path, 'mkdir') as mock_mkdir:
            uuid = uuid4()

            result = FileManagement.generate_file(uuid, 'filename')
    ...
    def test_remove_file(self, mock_get_root) -> None:
        with patch('shutil.rmtree') as mock_rmtree:
            ...
            mock_rmtree.assert_called_once_with(Path('/tmp', get_settings().upload_folder, 'uuid'))
            mock_get_root.assert_called_once()
    ...
    def test_list_all_files(self, mock_get_root, mock_get_file_directory) -> None:
        with patch.object(Path, 'iterdir') as mock_iterdir:
            with self.assertLogs(FileManagement.logger, level='INFO') as logs:
                ...
                self.assertEqual(logs.output[0], 'INFO:image_service:file with path: sentinel.file found')
                self.assertEqual(logs.output[1], 'INFO:image_service:file with path: sentinel.second_file found')
                self.assertEqual(logs.output[2], 'INFO:image_service:Found a total of 2 files')
                mock_get_root.assert_called_once()
